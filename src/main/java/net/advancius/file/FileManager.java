...
public class FileManager {
    ...
    @FlagManager.FlaggedMethod(flag = DefinedFlag.GAMES_PLUGIN_LOAD, priority = -5)
    public static void load() {
        AdvanciusGames.getInstance().getDataFolder().mkdirs();
    }
    ...
    public static InputStream getSourceFileStream(String path) {
        return AdvanciusGames.getInstance().getResourceAsStream(path);
    }

    public static File getServerFile(String path) {
        return new File(AdvanciusGames.getInstance().getDataFolder(), path);
    }
